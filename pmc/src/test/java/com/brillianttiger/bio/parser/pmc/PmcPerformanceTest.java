...
/**
 * PmcPerformanceTest / PMC 성능 테스트
 *
 * KR: PMC 파서의 성능 측정 및 최적화 검증
 * EN: Performance measurement and optimization verification for PMC parser
 *
...
class PmcPerformanceTest {
    ...
    void testStreamingPerformanceWithLargeDataset() throws Exception {
        System.out.println("========================================");
        System.out.println("PMC Streaming Performance Test");
        System.out.println("Target: 30,000+ articles");
        System.out.println("========================================\n");

        // 실제 PMC tar.gz 파일 경로 (여러 파일 검색)
        Path xmlFile = findLargePmcFile();

        if (!Files.exists(xmlFile)) {
            System.out.println("⚠️  대용량 PMC 파일이 없습니다.");
            System.out.println("📥 다운로드 방법:");
            System.out.println("   1. https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_comm/xml/ 접속");
            System.out.println("   2. tar.gz 파일 다운로드 (30,000+ articles)");
            System.out.println("   3. test-data/pmc/oa_comm/ 디렉토리에 저장");
            System.out.println("\n❌ 실제 대용량 파일이 필요합니다. SKIPPED\n");
            return; // 대용량 파일이 없으면 스킵
        }

        PmcXmlParser parser = new PmcXmlParser();
        PerformanceMetrics metrics = new PerformanceMetrics("Streaming - Large Dataset");
        ...
        long parsedCount = parser.parseStream(xmlFile, article -> {
            ...
            if (article.getFront() != null && article.getFront().getArticleMeta() != null) {
                article.getFront().getArticleMeta().getArticleIds(); // Article IDs 접근
            }
        ...
        // Assertions (테스트 파일 크기에 따라 조정)
        assertTrue(count.get() > 0, "Should parse at least one article");
        assertTrue(metrics.throughput > 100, "Throughput should be >100 articles/sec");
        ...
        // 리포트 생성
        generatePerformanceReport("pmc-streaming-performance", allMetrics);

        System.out.println("✅ PMC 스트리밍 성능 테스트 통과");
    }
    ...
    void testGzipVsNonGzipPerformance() throws Exception {
        System.out.println("========================================");
        System.out.println("PMC GZip vs Non-GZip Performance Test");
        System.out.println("========================================\n");

        // GZip 파일 찾기
        Path gzipFile = findGzipPmcFile();

        if (!Files.exists(gzipFile)) {
            System.out.println("⚠️  GZip PMC 파일이 없습니다.");
            System.out.println("📥 src/test/resources/pmc/ 에 .xml.gz 파일 필요");
            System.out.println("SKIPPED\n");
            return;
        }

        // Non-GZip 파일 생성 (압축 해제)
        Path uncompressedFile = Paths.get(gzipFile.toString().replace(".gz", ""));
        ...
        PerformanceMetrics gzipMetrics = measureParsingPerformance(
            parser, gzipFile, "PMC GZip Compressed"
        );
        ...
        PerformanceMetrics uncompressedMetrics = measureParsingPerformance(
            parser, uncompressedFile, "PMC Uncompressed"
        );
        ...
        // 리포트 생성
        generatePerformanceReport("pmc-gzip-vs-nongzip", allMetrics);

        System.out.println("\n✅ PMC GZip vs Non-GZip 성능 테스트 통과");
    ...
    void testMemoryUsageWithLargeDataset() throws Exception {
        System.out.println("========================================");
        System.out.println("PMC Memory Usage Measurement Test");
        System.out.println("========================================\n");
        ...
        if (!Files.exists(xmlFile)) {
            ...
            System.out.println("📥 다운로드 방법:");
            System.out.println("   1. https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_comm/xml/ 접속");
            System.out.println("   2. tar.gz 파일 다운로드");
            System.out.println("   3. test-data/pmc/oa_comm/ 디렉토리에 저장");
            System.out.println("\n❌ 실제 대용량 파일이 필요합니다. SKIPPED\n");
            return; // 대용량 파일이 없으면 스킵
        }
    ...
    void testThroughputBenchmark() throws Exception {
        System.out.println("========================================");
        System.out.println("PMC Throughput Benchmark Test");
        System.out.println("========================================\n");
        ...
        // 시나리오 1: 최소 처리 (Article ID만 접근)
        System.out.println("시나리오 1: 최소 처리 (Article ID only)");
        ...
        parser.parseStream(xmlFile, article -> {
            ...
            if (article.getFront() != null && article.getFront().getArticleMeta() != null) {
                article.getFront().getArticleMeta().getArticleIds();
            }
        });
        ...
        parser.parseStream(xmlFile, article -> {
            ...
            if (article.getFront() != null && article.getFront().getArticleMeta() != null) {
                ...
                if (meta.getTitleGroup() != null) {
                    meta.getTitleGroup().getArticleTitle();
                }
                meta.getContribGroups();
            }
        });
        ...
        // Assertions
        assertTrue(minimalMetrics.throughput > 100, "Minimal processing should be >100 articles/sec");
        assertTrue(fullMetrics.throughput > 50, "Full processing should be >50 articles/sec");

        // 리포트 생성
        generatePerformanceReport("pmc-throughput-benchmark", allMetrics);
     ...
     * 대용량 PMC 파일 찾기
     */
    private Path findLargePmcFile() {
        // 여러 경로 시도 (대용량 파일만)
        String[] candidates = {
            BASE_DIR + "/oa_comm/pmc_oa_comm_xml.tar.gz",
            "src/test/resources/pmc/large_dataset.xml.gz",
            "src/test/resources/pmc/large_dataset.xml"
        };
        ...
        // 대용량 파일이 없으면 존재하지 않는 경로 반환
        return Paths.get(BASE_DIR + "/oa_comm/large_dataset.xml");
    }

    /**
     * GZip PMC 파일 찾기
     */
    private Path findGzipPmcFile() {
        String[] candidates = {
            BASE_DIR + "/oa_comm/*.tar.gz",
        ...
        return Paths.get("src/test/resources/pmc/full_article.xml.gz");
    }
    ...
    private PerformanceMetrics measureParsingPerformance(
        PmcXmlParser parser, Path xmlFile, String testName) throws Exception {
     ...
     */
    private void processFullArticle(JatsArticle article) {
        if (article.getFront() == null || article.getFront().getArticleMeta() == null) {
            return;
        }
        ...
        if (meta.getTitleGroup() != null) {
            meta.getTitleGroup().getArticleTitle();
            meta.getTitleGroup().getSubtitles();
        }
        ...
        // Funding
        meta.getFundingGroups();

        // Body
        if (article.getBody() != null) {
            article.getBody().getSections();
        }

        // Back
        if (article.getBack() != null) {
            article.getBack().getRefLists();
        }
    }
    ...
    private void generatePerformanceReport(String testName, List<PerformanceMetrics> allMetrics)
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(reportFile.toFile()))) {
            writer.write("# PMC Performance Test Report\n\n");
